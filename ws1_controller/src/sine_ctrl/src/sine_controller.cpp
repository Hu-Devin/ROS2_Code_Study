...
namespace sine_controller {
  ...
  private:
    ...
    float t_; // Time accumulator for sine wave calculation
  ...
  public:
    ...
    if(t_ == 0.0) {
        RCLCPP_INFO(get_node()->get_logger(), "Starting sine wave control");
      initial_joint_position_[0] = static_cast<float>(joint_state_interfaces_[0][0].get().get_optional<double>().value());
      initial_joint_position_[1] = static_cast<float>(joint_state_interfaces_[1][0].get().get_optional<double>().value());
      ...
      for (int i = 0; i < 2; i++) {
        ...
        (void)command_interfaces_[i].set_value(static_cast<double>(desired_joint_positions_[i])); // Apply position command
      }
